...
extern SYS_CONF g_conf;
extern int g_vda_open;
extern int g_film_onoff_status ;
extern int g_film_switch_event ;
extern int g_alarm_onoff_status;
extern int g_alarm_switch_event;
...
extern void save_md_log(int alarm_count);
HI_VOID *SAMPLE_COMM_VDA_MdGetResult(HI_VOID *pdata)
{
    int ix=0;
    long int dark_times;
    long int light_times;
    int alarm_count;
    HI_S32 s32Ret;
    VDA_CHN VdaChn;
    ...
    while (HI_TRUE == pgs_stMdParam->bThreadStart)
    {
        ...
        else
        {
            if (FD_ISSET(VdaFd, &read_fds))
            {
                ...
                alarm_count = stVdaData.unData.stMdData.u32AlarmPixCnt;
		//printf("alarm_count = %d\n",alarm_count);

				g_alarm_count = alarm_count;
				ix++;
				if((g_vda_open)&&(ix>10)) //save once in ten times
				{
					 save_md_log(alarm_count);
					 ix = 0;
				}
                if(alarm_count <= g_conf.film_dark_maxval)
                {
                      dark_times++;
                      light_times=0;
                 
                }
                if(alarm_count > g_conf.film_light_minival)
                {
                      light_times++;
                      dark_times=0;
                ...
                //
                if(dark_times>g_conf.film_dark_detect_times)
                { 
                    if(g_alarm_onoff_status==1)
                    {
                         g_alarm_switch_event=1;
                         printf("Alarm converting to close: %d\n", dark_times);
                    }
                    g_alarm_onoff_status = 0;
                    dark_times = 0;
                    light_times=0;
                    
                   
                }
                if(light_times>g_conf.film_light_detect_times)
                {
                   if(g_alarm_onoff_status ==0)
                   {
                         g_alarm_switch_event = 1;
                         printf("Alarm  converting to open: %d\n",light_times);
                   }

                   g_alarm_onoff_status = 1;
